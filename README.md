...
- `config.mode`
  - режим профилирования PostgreSQL-источника
  - `full` или пустое значение: точные агрегации по всей таблице
  - `sampled`: статистика считается по выборке строк, быстрее на больших таблицах, но значения приблизительные
  - `schema_only`: сохраняются только таблицы, представления, колонки и типы, без статистики по значениям
